...
def create_ping_func(**kwargs):
    ...
    def ping_func(source, target):
        ...
        try:
            ...
            log.debug('there is no such source')
        ...
        if not mylink:
            log.debug('there is no link to me in source')
        ...
        # stupid workaround because django returns tuple
        # instead of RegexURLPattern
        registered = False
        for name in kwargs:
            if resolver.reverse_dict[name] in url_signatures:
                ...
                break
        if not registered:
            log.debug('target is not registered')
        ...
        try:
            Pingback.objects.get(url=source, content_type=content_type,
                                 object_id=obj.id)
            log.debug('target has already registered pingback')
